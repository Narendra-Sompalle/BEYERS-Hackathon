...
def _log(phase: str, msg: str):
    """Print + log so it shows in both local terminal and CloudWatch."""
    line = f"[COMMANDER|{phase}] {msg}"
...
def parse_alarm(event: dict) -> dict:
    ...
    """
    _log("DETECT", "Parsing alarm event...")
    result = parse_alarm_event(event)
    _log("DETECT", f"Incident: {result.get('incident_id')} | Service: {result.get('service')} | "
         f"Metric: {result.get('metric_name')} | Threshold: {result.get('threshold')} | "
         f"Values: {result.get('current_values')} | Detected: {result.get('detected_at')}")
    return result
...
def compute_confidence_score(
    ...
    """
    _log("DECIDE", f"Computing confidence: logs={logs_confidence}, metrics={metrics_confidence}, "
         f"deploy={deploy_confidence}, overlap={has_timestamp_overlap}, "
         f"config_match={has_config_match}, failed={failed_agents}")
    ...
    _log("DECIDE", f"Score: base={result['base_score']} + boost={result['evidence_boost']} "
         f"- penalty={result['failure_penalty']} = {result['base_confidence']}")
    return result
...
def generate_rca_markdown(
    ...
    """
    _log("REPORT", f"Generating RCA for {incident_id}")
    _log("REPORT", f"Root cause: {root_cause}")
    _log("REPORT", f"Confidence: {confidence:.0%} | Action: {recommended_action}")
    _log("REPORT", f"Evidence chain: {evidence_chain}")
...
"""
    _log("REPORT", "RCA markdown generated successfully")
    return report
